...
import jxl.write.*;
import net.app315.hydra.example.server.exception.ExcelException;
...
//            throw new ExcelException("数据源中没有任何数据");
     ...
     * @Description : 导出Excel（可以导出到本地文件系统，也可以导出到浏览器，可自定义工作表大小）
     */
    public static <T> void listToExcel(List<T> list, Map<String, String> fieldMap, String sheetName, int sheetSize, OutputStream os) throws ExcelException {
...
//            throw new ExcelException("数据源中没有任何数据");
     ...
     * @Description : 导出Excel（导出到浏览器，工作表的大小是2003支持的最大值:65535）
     */
    public static <T> void listToExcel(List<T> list, Map<String, String> fieldMap, String sheetName, HttpServletResponse response) throws ExcelException {
    ...
    private static <E> void fillSheet(WritableSheet sheet, List<E> list, Map<String, String> fieldMap, int firstIndex, int lastIndex) throws Exception {
        ...
        // 填充表头
        for (int i = 0; i < cnFields.length; i++) {
            Label label = new Label(i, 0, cnFields[i],writableCellFormat);
            sheet.addCell(label);
        }
