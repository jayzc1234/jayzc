...
import static net.app315.hydra.example.server.config.tenant.MyTenantConstants.*;
...
public class MyTenantSqlParser extends AbstractJsqlParser {
    ...
    protected boolean validSysAndOrgIsExists(Expression expression) {
        ...
        boolean isTrue = sqlWhere.contains(VALID_SYS_ID) || sqlWhere.contains(VALID_ORGANIZATION_ID);
        if (isTrue) {
            log.info("sql中已存在sys_id或organization_id的校验，过滤租户查询");
        }
